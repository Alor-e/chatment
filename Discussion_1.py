...
# Add the argument for setting the split number range with a shorter command name as well
parser.add_argument('-sr', '--split-range', type=int, nargs=2, help='Range of split numbers, inclusive. Example: -sr 1 5 or --split-range 1 5')

# Add the argument for setting the split number range with a shorter command name as well
parser.add_argument('-rn', '--run-number', type=int, nargs=2, help='Range of split numbers, inclusive. Example: -rn 1 5 or --run-number 1 5')

# Parse the command line arguments
args = parser.parse_args()

# Set rc.DATASET based on the command line argument
if args.dataset:
    rc.DATASET = args.dataset

# Use the provided split range if available
if args.split_range:
    split_start, split_end = args.split_range
else:
    # Default range values if --split-range is not used
    split_start, split_end = 1, 7  # Adjust these defaults as needed

# Use the provided split range if available
if args.run_number:
    run_start, run_end = args.run_number
    rc.NUMBER_OF_RUNS = run_end
else:
    # Default range values if --split-range is not used
    run_start, run_end = 0, 7  # Adjust these defaults as needed
...
print("Loading model outside...")
model = SentenceTransformer(rc.MODEL_LOCATION)

for split_number in range(split_start, split_end + 1):
    percentage = 'D1_' + str(split_number * 10)
    rc.TRAINING_DATA_PERCENT = split_number * 0.1
    rc.TESTING_DATA_PERCENT = 0.8 - rc.TRAINING_DATA_PERCENT

    for i in range(run_start, rc.NUMBER_OF_RUNS):
        rc.OUTPUT_PATH = './results/' + rc.DATASET + '_' + percentage + '/'
        rc.LOGS_PATH = './logs/' + rc.DATASET + '_' + percentage + '/'
        dataset_logs = rc.DATASET + '_' + percentage
        ...
        # Log out intent dict
        log_json(loader.intent_dict, dataset_logs, i, "intent_dict.json")
        ...
        grouper = sem_grouper.SemanticGrouper(loader.training_x, loader.training_y, loader.testing,
                                            loader.intent_dict,
                                            transformer=rc.TRANSFORMER, model=model)
        new_pos, new_labels = grouper.get_similar_sentences(threshold=rc.SEM_GROUP_THRESHOLD)

        log_json({"new_pos": new_pos, "new_labels": new_labels}, dataset_logs, i, "grouper_output.json")
        ...
        log_json({'intent_mapping': loader.intent_dict, 'training_x': loader.training_x, 'training_y': loader.training_y, 'testing': testing, 'testing_y': testing_y, 'validation': loader.validation, 'validation_y': loader.validation_y, 'pruner_training_x': pruner_training_x, 'pruner_training_y': pruner_training_y}, dataset_logs, i, "loader_dict.json")
        ...
        log_json({"number of labelling functions": len(lfs), "lf_words_labels": [{"words": item.words, "labels": item.label} if hasattr(item, 'words') else {"entity": item.entity, "labels": item.label} if hasattr(item, 'entity') else {"word_entity_combo": item.word_entity_combo, "labels": item.label} if hasattr(item, 'word_entity_combo') else {"ml": None, "labels": item.label} for item in lfs]}, dataset_logs, i, "lfs_output.json")
        ...
        lf_summary_df = LFAnalysis(lf_matrix).lf_summary(pruner_training_y) # No need for this?
        ...
        try:
            lf_summary_df.to_csv(f'./logs/{rc.DATASET}_{percentage}/run_{i}/lf_summary.csv', index=False)
        except OSError:
            os.makedirs(f'./logs/{rc.DATASET}_{percentage}/run_{i}', exist_ok = True)
            lf_summary_df.to_csv(f'./logs/{rc.DATASET}_{percentage}/run_{i}/lf_summary.csv', index=False)

        # Log out intent dict
        output_dir = f"./logs/{rc.DATASET}_{percentage}/run_{i}"
        Path(output_dir).mkdir(parents=True, exist_ok=True)
        ...
        log_json({"preds": preds.tolist()}, f"{rc.DATASET}_{percentage}", i, "mlv_output_t.json")
        log_json({"accuracy": majority_voter.get_accuracy(testing_y, preds)}, f"{rc.DATASET}_{percentage}", i, "mlv_accuracy_output_t.json")
        ...
        log_json({"preds": preds.tolist()}, f"{rc.DATASET}_{percentage}", i, "mlv_output.json")
        log_json({"accuracy": majority_voter.get_accuracy(testing_y + loader.validation_y, preds)}, f"{rc.DATASET}_{percentage}", i, "mlv_accuracy_output.json")

        auc_score = calculate_average_auc(testing_y + loader.validation_y, preds)
        log_json({"auc_score": auc_score}, f"{rc.DATASET}_{percentage}", i, "mlv_auc_score.json")
...
def get_average_auc_score(prune_dir_pattern='./logs/*D1*/'):
    average_scores = {}
